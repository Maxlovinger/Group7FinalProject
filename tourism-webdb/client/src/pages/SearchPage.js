...
export default function SearchPage() {
  ...
  return (
    <Box>
      <Typography variant="h4" sx={{ mb: 1, color: 'white' }}>🔍 Search Airports</Typography>
      <Typography sx={{ color: 'grey.400', mb: 4 }}>
        Find airports by city name or country code.
      </Typography>
      ...
      <Paper elevation={0} sx={{ p: 3, mb: 4, border: '1px solid #1e2d4a', borderRadius: 2 }}>
        <Grid container spacing={3} alignItems="flex-end">
          <Grid item xs={12} sm={5}>
            <TextField
              fullWidth label="City name" variant="outlined" size="small"
          ...
          <Grid item xs={12} sm={4}>
            <TextField
              fullWidth label="Country code (e.g. US, FR, GB)" variant="outlined" size="small"
          ...
          <Grid item xs={12}>
            <Box sx={{ display: 'flex', gap: 2 }}>
              <Button
                ...
                sx={{ bgcolor: 'primary.main', color: '#0a0f1e', fontWeight: 700, '&:hover': { bgcolor: '#00b899' } }}
              >
                Search
              </Button>
              <Button
                variant="outlined" startIcon={<CasinoIcon />}
                onClick={handleSurprise} disabled={loading}
                sx={{ borderColor: 'secondary.main', color: 'secondary.main', '&:hover': { borderColor: '#ff8555' } }}
              >
                Surprise Me!
              </Button>
            </Box>
        ...
        <>
          ...
          {results.length === 0 ? (
            <Typography sx={{ color: 'grey.600', textAlign: 'center', py: 6 }}>
              No airports matched your search. Try a different city or country code.
            </Typography>
          ) : (
              ...
              {results.map((airport, i) => (
                <Grid item xs={12} sm={6} md={4} key={i}>
                  <Paper elevation={0} sx={{
                    p: 2.5, border: '1px solid #1e2d4a', borderRadius: 2,
